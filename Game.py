...
class Game(object):
    ...
    def draw_hexagon(self, x, y, color=(200, 200, 200)):
        angle = 0
        points = []
        for _ in range(6):
            x_i = x + self.board_config["hex_radius"] * math.cos(math.radians(angle))
            y_i = y + self.board_config["hex_radius"] * math.sin(math.radians(angle))
    ...
    def draw_hexgrid(self, height, width, hex_radius) -> list[Hex]:
        ...
        for row in range(2 * row_step, height - 2 * row_step, row_step):
            ...
            for col in range(1 * col_step, width - 6 * col_step, col_step):
                self.draw_hexagon(col + step * 1.5 * hex_radius, row)
